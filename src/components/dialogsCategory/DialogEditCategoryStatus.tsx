...
export default function DialogEditCategoryStatus({ categoryId, authToken, status }: DialogEditCategoryStatus) {
  ...
  const { isUpdated, isUpdating, error } = usePut(`${process.env.NEXT_PUBLIC_BASE_URL}/category/edit/${categoryId}`, {CATEGORY_DELETED: false}, posted, {
    headers: {
    ...
    <DialogContent className="w-[28rem] max-w-[90vw]">
      <DialogHeader>
        ...
        <DialogDescription className="text-center">
          Clicar no botão deixará a categoria disponivel novamente.
        </DialogDescription>
      </DialogHeader>
      <Form {...form}>
